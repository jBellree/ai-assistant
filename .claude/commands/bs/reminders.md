---
description: Show Apple Reminders — ask B which slice first, then fetch just that slice
argument-hint: [overdue|today|week|flagged|list <name>|everything]
---
...
The script is allowlisted in [.claude/settings.json](../../settings.json), so fetches don't prompt. It emits JSON on stdout; keep the output in a Bash tool call and let Claude read it directly.
...
Out of scope for this command. If B asks to undo a tick-off or delete a reminder, point him to run `~/code/apple-mcp/bin/reminders-helper uncomplete <id>` or `delete <id>` manually, or invoke it one-off with the id from the last slice.

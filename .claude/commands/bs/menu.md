...
Use the Glob tool to find `.claude/commands/bs/*.md` (relative to the current working directory). For each file:

1. Read its frontmatter `description` field.
2. Output one line: `/bs:<name>, <description>` (use a comma, never em/en dashes, per communication-style rules).
...
If a command file has no `description` in frontmatter, output `/bs:<name>, (no description)`.
